...
# CIL CONFIG
MODE="derpp" # finetune ,native_rehearsal,joint, rwalk, icarl, rainbow_keywords, ewc, bic, derpp
...
TRANS="None" # multiple choices: mixup specaugment
TOPK=1
N_WORKER=10
...
python main.py --mode derpp --mem_manage $MEM_MANAGE --exp_name $EXP \
--dataset $DATASET \
...
--feature_size $FEAT_SIZE $distilling --joint_acc $JOINT_ACC --minibatch_size $MINIBATCH_SIZE
...
# python main.py --mode finetune --mem_manage $MEM_MANAGE --exp_name $EXP \
# --dataset $DATASET \
# --stream_env $STREAM  $INIT_MODEL $INIT_OPT --topk $TOPK \
# --n_tasks $N_TASKS --n_cls_a_task $N_CLS_A_TASK --n_init_cls $N_INIT_CLS \
# --rnd_seed $RND_SEED \
